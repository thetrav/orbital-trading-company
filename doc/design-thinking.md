...
* I think chasing auto placement is a fools errand, instead we should let the player with the highest bond share be the Mayor and they can select the positioning of the expansion once it can be afforded.
  + built. Bonds and the mayoral election are in `doc/nauvis_voting.md` — the mayor is elected rather than being whoever holds the most bonds — and `scripts/nauvis_siting.lua` hands them the footprint to place once the expansion is paid for. It must be dry, clear, out of the enemy's way and in reach of the power grid; trees, rocks and cliffs are levelled. `district.lua`'s packer still lays out the starting fixtures and a company's given facilities, which cannot wait for an election.
